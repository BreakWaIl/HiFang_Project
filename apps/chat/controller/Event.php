                ...
                foreach ($payload['events'] as $event) {
                    ...
                    if ($event['name'] == 'channel_removed') {
                        ...
                        if (strpos($event['channel'], 'user') === 0) {
                            ...
                            $user = Member::table('yf_hf_member')->where('id', $id)->find();
                            if (!$user) {
                                $arr = [];
                                $arr['id'] = $id;
                                ...
                                $arr['app_key'] = $app_key;
                                $arr['status'] = 'offline';
                                $res = Member::table('yf_hf_member')->insert($arr);
                            }

                            $re = Member::table('yf_hf_member')->where('id', $id)->update(['logout_timestamp' => time(), 'status' => 'offline']);
                        }
                    ...
                    //     if(strpos($event['channel'],'user')=== 0){
                    //          $id = str_replace('user', '', $event['channel']);
                    //          $user = Member::table('yf_hf_chat_user')->where('uid', $id)->find();
                    //          if(!$user){
                    //             $logtime =0;
                    //          }else{
