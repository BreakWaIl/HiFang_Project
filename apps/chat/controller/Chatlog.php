        ...
        if (!isset($get['hid'])) {
            ...
            if($type == "friend"){
               // $chatdata = User::query("select * from message where (`from` ='{$id}' or `to` ='{$id}') and (`from` ='{$mine}' or `to` ='{$mine}') and type='{$type}' and `app_key`='{$app_key}' order by `mid` desc limit 11");
                ...
                $chatdata =Member::table("yf_hf_chat_message")->where('from',['=',$id],['=',$mine],"or")->where('to',['=',$id],['=',$mine],"or")->where(['type'=>$type,'app_key'=>$app_key])->order("mid desc")->limit(11)->select();
            }else{
               // $chatdata = User::query("select * from message where (`to` ='{$id}') and type='{$type}' and `app_key`='{$app_key}' order by `mid` desc limit 11");
               
                $chatdata =Member::table("yf_hf_chat_message")->where('to',$id)->where(['type'=>$type,'app_key'=>$app_key])->order("mid desc")->limit(11)->select();
            ...
            if($type == "friend"){
                 ...
                 $chatdata =Member::table("yf_hf_chat_message")->where('from',['=',$id],['=',$mine],"or")->where('to',['=',$id],['=',$mine],"or")->where(['type'=>$type,'app_key'=>$app_key])->where('mid','<',$msgid)->order("mid desc")->limit(11)->select();

            }else{
             

                $chatdata =Member::table("yf_hf_chat_message")->where('to',$id)->where(['type'=>$type,'app_key'=>$app_key])->where('mid','<',$msgid)->order("mid desc")->limit(11)->select();
            ...
            if($type == "friend"){
                ...
                $chatdata =Member::table("yf_hf_chat_message")->where('from',['=',$id],['=',$mine],"or")->where('to',['=',$id],['=',$mine],"or")->where(['type'=>$type,'app_key'=>$app_key])->where('mid','>',$msgid)->order("mid")->limit(11)->select();


            }else{
              
              $chatdata =Member::table("yf_hf_chat_message")->where('to',$id)->where(['type'=>$type,'app_key'=>$app_key])->where('mid','>',$msgid)->order("mid")->limit(11)->select();
