...
class Base extends Model
{
     ...
     * @return [type]        执行结果
     */
    public function editMusic($data, $confirm=false)
    {
        $this->allowField(true);
     ...
     * @auther zyt
     */
    public function getListByPages($alias,$join,$condition,$like,$type,$map,$field,$order='create_time desc',$page_size=null)
    {
        $paged     = input('param.paged',1);//分页值
        if (!$page_size) {
            $page_size = config('admin_page_size');
        ...
        $query     = $this->alias($alias)
                        ->join($join,$condition,$type);
        $rule = '%[KEYWORD]%';
        if (strpos($rule, '[KEYWORD]')!==false) {
            $keyword     = input('param.keyword',false);//关键字
            if (!empty($keyword)) {
                $rule = str_replace('[KEYWORD]', $keyword, $rule);
                $query = $query->where($like,'like',$rule);
            }
        }
        $list = $query->where($map)
                        ->field($field)
                        ->order($order)
                        ->page($paged,$page_size)->select();
        $total = $this->where($map)->count();
        return [$list,$total];
    }
