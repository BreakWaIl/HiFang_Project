...
class CKFinder_Connector_Utils_Misc
{
    public static function getErrorMessage($number, $arg = "") {
        ...
        if ($number) {
            if (!empty ($GLOBALS['CKFLang']['Errors'][$number])) {
                ...
                if(!empty($_REQUEST['resource_space_size'])){

                    $errorMessage = str_replace("%2", $_REQUEST['resource_space_size'], $errorMessage);

                    if(!empty($_REQUEST['remain_space_size'])){
                        $errorMessage = str_replace("%3", $_REQUEST['remain_space_size'], $errorMessage);
                    }else{
                        $errorMessage = str_replace("%3", 0, $errorMessage);
                    }
                }
            } else {
    ...
    public static function returnBytes($val) {
        ...
        //todo zw 这块代码有点问题，如下修改
         /*   $last = strtolower($val[strlen($val)-1]);
            $valSize = substr($val,0,strlen($val)-1);
             switch($last) {
        ...
        switch($last) {
            // The 'G' modifier is available since PHP 5.1.0
            case 'g':
                $val = $valSize * pow(2,30);
                break;
            case 'm':
                $val = $valSize * pow(2,20);
                break;
            case 'k':
                $val = $valSize * pow(2,10);
                break;
        }
        return $val;
    }
